...
void PrintBottomView(Tree* root)
{
    ...
    while(!q.empty())
    {
        //here we make a temp that contain node and int value and give it the q.fornt() value
        ...
        int dis=temp.second;
        //here every time when we entry in map we overwrite the value and dis
    ...
    cout<<"Our bottom view elements are"<<endl;
    for(auto i: mapping)
    {
        cout<<i.second;
        cout<<endl;
